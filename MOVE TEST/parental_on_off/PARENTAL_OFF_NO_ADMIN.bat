...
set /a attempts+=1
echo.
echo [VERIFY !attempts!] Checking unrestricted status...

REM Test 1: FamilySafety gone
powershell -c "!(Test-Path 'HKCU:\\Software\\Policies\\Microsoft\\Windows\\FamilySafety')" | find "True" >nul
if errorlevel 1 goto pass1
goto verify_loop

:pass1
REM Test 2: Edge policies gone
powershell -c "!(Test-Path 'HKCU:\\Software\\Policies\\Microsoft\\Edge')" | find "True" >nul  
if errorlevel 1 goto pass2
goto verify_loop

:pass2
echo [PASS !attempts!] ALL POLICIES DELETED ✓
echo [FINAL] Test Bing access now...
start microsoft-edge:https://bing.com
echo 🔥 UNRESTRICTED CONFIRMED NO JOKE FR!
pause
